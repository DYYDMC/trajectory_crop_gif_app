...
class TrajectoryCropApp:
    ...
    def _build_ui(self) -> None:
        ...
        tk.Button(row1, text="Continue", command=self.continue_with_params).pack(side=tk.LEFT, padx=4)
        tk.Button(row1, text="Save Accept", command=self.accept_result).pack(side=tk.LEFT, padx=4)
        tk.Button(row1, text="Change Something", command=self.change_something).pack(side=tk.LEFT, padx=4)

        tk.Button(row2, text="Approach Trajectory", command=self.use_approach_trajectory).pack(side=tk.LEFT, padx=4)
        tk.Button(row2, text="Nonapproach Trajectory", command=self.use_nonapproach_trajectory).pack(side=tk.LEFT, padx=4)
        tk.Button(row2, text="Stationary Trajectory", command=self.use_stationary_trajectory).pack(side=tk.LEFT, padx=4)
